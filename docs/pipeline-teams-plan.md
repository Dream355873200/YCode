...
### Teams v2 修正设计：用户创建 + 成员持久化（重要修订）

原 L1 设计（主 agent 运行时组队、成员随分派生灭）有两处被推翻：

1. **团队由用户创建**（不是主 agent）——递归风险从能力面上消失（成员
   工具白名单里没有 pipeline/team 创建工具），团队跨会话存活，生命周期
   不与主 agent 的单次 run 绑定。
2. **成员 = 持久化会话**——跨分派累积记忆（"成员记得上周干过什么"），
   执行历史像主 agent 一样落盘可回放；不是短命 worker。

```
.yume/teams/<name>/
├── team.json        # 定义：leader 角色卡 + 成员角色卡 + 工具集 + trust
├── sessions/<member>.jsonl   # 每成员持久会话（跨分派累积）
└── memory/          # 团队共享记忆（决议/交接）
```

**主 agent 获得团队交互工具**（而非创建权）：

```
team_dispatch(team, member|leader, task)  # 发任务（走会话 Gate 审批）
team_status(team)                         # 成员状态一览
team_read(team, member)                   # 读成员产出/历史摘要
```

**信息流**：用户 ⇄ leader ⇄ 成员（单向清晰）；实时可见 = 每成员 loop
事件经 SSE 透出 → 右栏「团队」群聊流；点成员 → 完整执行时间线（持久化
...
### 群聊 = 团队消息总线（事件驱动，含插话式指挥与 @路由）

群聊不是闲聊室，是**团队消息总线的视图**：`.yume/teams/<name>/chat.jsonl`，
每条带发送者归属；安静时群里没消息（事件驱动，不刷屏）。

| 群里谁发的 | 路由 |
|---|---|
| 用户（无 @） | = leader 的插话（leader 运行中走 steering，未运行注入其会话） |
| 用户（@成员） | 路由给该成员（运行中 → steering 注入；未运行 → 待办/启动） |
| leader | 正式分派走 dispatch；运行中成员的纠偏走 steering（插话式指挥） |
| 成员 | 汇报 → leader；最终产出同时交回分派方 |
| pipeline 节点 | 交接确认进群聊（团队上下文运行 pipeline 时） |
| 系统 | 成员启动/完成/失败 |

成员运行中的 thinking/tool 明细**不进群聊**（太吵）——点成员头像进入该
成员的执行时间线（持久化可回放），与主对话差异化设计保留。

**leader 的两种指挥**：
1. 正式分派（dispatch）：拆解出子任务 → 成员执行 → 汇报
2. 插话式指挥（steer）：成员运行中纠偏（"跳过第3步直接看结果"）——
   经 steering 通道注入成员运行，不打断其执行流

### 角色卡 vs mode：正交的两个维度

- **角色卡 = 身份与能力面**：「你是谁、会什么、能用什么工具、向谁汇报」
  （system prompt + 工具白名单 + 汇报关系），**持久身份**，跨任务不变。
- **mode = 审批策略**：「做事时需不需要人批准」（default / accept_edits /
  plan / bypass），**运行策略**，可随时切。

两者正交且都需要：同一个「测试工程师」角色卡，配手动审批 = 每次写文件
...
YCode 从「开发工具」演进为「通用 agent 平台」：每个用户群体 = 一个场景包
（模式定义 + 插件组合 + 角色卡模板），机制全部复用（modes/plugins/角色卡/
pipeline/teams/群聊总线）。

### 场景映射

| 群体 | 模式 | 工具集 | 角色卡 | pipeline/teams 形态 |
|---|---|---|---|---|
| 开发者（现有） | code/flutter | 文件/Bash/CUA | 技术人设 | 测试矩阵/重构流水线 |
| 日常聊天 | chat | 空/极简 | 人设+长期记忆 | 多角色群聊（多 bot 同群） |
| 游戏（Minecraft 等） | game | 游戏协议工具 | bot 人设+分工 | 建造队/探索队协作 |
| 教育/写作 | tutor | 资料/文档工具 | 导师人设 | 教案→讲解→练习生成 |

**chat 场景：teams 群聊形态就是产品本身**——不同人设的 bot 同群互动，
角色卡（人设）+ 群聊总线 + @路由全部复用。

### 四个真实缺口

1. **无项目目录的场景**：chat/game 不绑定工作目录——`session-map → dir`
   需可空化，WorkDir 机制可空化（chat 模式的文件工具不出现）
2. **长期对话记忆**：chat 的核心是「记得用户」——GoAgent session_memory
   是压缩时的一次性摘要；需要主动的对话记忆抽取器（定期提取用户偏好/
   重要事实存档，注入后续会话）
3. **角色卡即人设**：机制完全复用（system prompt + 工具白名单），内容
   性质变为 bot 人设（性格/语气/边界）
4. **场景包分发**：每个群体 = 模式定义 + 插件组合 + 角色卡模板，清单
   内容层面的工作（机制零改动）

### Pipeline/Teams 在非工作场景的形态

- **chat 多 bot 群**：teams 群聊视图 = 产品形态本身（多角色 bot 同群互动，
  @路由响应）；pipeline 用得少（对话无数据变换流）
- **game bot**：Minecraft 多 bot 协作——pipeline（勘探→规划→并行建造→
  验收，队列数据 = 游戏坐标/方块指令）与 teams（长期分工：建造者/守卫/
  后勤）都有真实用例
- 教育场景：pipeline（教案→讲解→练习生成）+ tutor 角色卡
