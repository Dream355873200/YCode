---
name: visual-judge
description: "渲染成品（pptx/docx/xlsx/pdf/海报/图表）页图的唯一视觉验收关：代替你自己翻页图做用户验收裁决，逐页返回 pass/fail + 证据 JSON。只读，只能 Read 预渲染的页 PNG——先把成品渲染成 PNG 再把路径派给它；verdict 就是视觉门的结果，不要自己再复判一遍。"
